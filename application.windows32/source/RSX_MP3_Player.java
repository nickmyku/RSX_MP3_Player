...
public class RSX_MP3_Player extends PApplet {
...
public void draw()
{
  ...
  if(Randomized && !Random)  {
    Randomized = false;
    SongNum = PApplet.parseInt(RandIndex.get(SongNum).toString());
  }
  ...
  rect(50, 550, 700, 10); 
  fill(255,0,0);
  rect(50, 550, PApplet.parseInt(700*(PApplet.parseFloat(progress)/PApplet.parseFloat(total))), 10);
...
public void mousePressed()  {
  ...
  if(mouseX>50 && mouseX<750)  {
    if(mouseY>545 && mouseY<600)  {
      song.cue(PApplet.parseInt(((PApplet.parseFloat(mouseX)-50)/700)*PApplet.parseFloat(song.length())));
    }
  }
...
public void createTrackList(String ArtistStr, String TrackStr, boolean FindSong)  {
  int index = 0;
  int SongIndex = 0;
  ...
  if(FindSong)  {
    ...
    SubSet = PApplet.parseInt(PApplet.parseFloat(SongIndex)/10);
...
public void changeSong(int index)  {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--hide-stop", "RSX_MP3_Player" };
    if (passedArgs != null) {
